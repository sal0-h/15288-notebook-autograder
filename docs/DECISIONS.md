...
| **Rubric review default**            | **On by default** in `AppConfig` / UI (`rubric_review: true`). Canonical **`experiment_data/**/config.yaml`** files produced by `scripts/build_experiment_layout.py` set **`rubric_review: false`** so research layouts skip the second LLM pass unless you re-enable it (avoids extra “softening” of criteria). |
| **Temperature**                      | **As low as the model allows** — `temperature_for_model()` uses **0** by default; **1** for early GPT‑5 ids (`gpt-5`, `gpt-5-mini`, `gpt-5-nano`, `*codex*`, …) and **0** for `gpt-5.2`+ dot releases when the API accepts it (verify with `test_openai_connection.py --probe-chat`).                                                                                                                                                                      |
