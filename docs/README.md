...
| `research/paper/` (local) | **Not in git.** SIGCSE Virtual 2026 ERT paper sources, figures, and metric CSV/JSON live only on disk under `research/paper/` after you clone. Lab totals: `python scripts/export_research_paper_metrics.py` (optional `--write-snapshot`); HW1 IRR: `python scripts/compute_irr.py`. Prompt-injection red-team: `python scripts/build_hw2_injection_bench.py` → `output/HW2_Injection_Bench_gpt_4_1/` and `output/HW2_Injection_Bench_gpt_4_1_mini/` — see **[research/README.md](../research/README.md)**. |
