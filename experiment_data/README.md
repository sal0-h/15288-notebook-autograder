...
The root `.gitignore` ignores all of `experiment_data/**` except this README, so cohort trees never enter git. The `_raw/` folders contain Gradescope exports with names, SID, and email — they must stay local. The canonical `human_grades.csv` only carries `anon_id`, totals, and per-qid scores, but is still gitignored; do not commit raw or lightly anonymized exports.

**S24 hand-canonical labs** (under `experiment_data/S24/LabTest_*`) do **not** use `build_experiment_layout.py` notebook rewriting. Use `scripts/canonicalize_experiment_lab.py` so `solution.ipynb` stays a byte copy of `_raw/*_sol.ipynb`, with optional two-capture `question_regex` for dash-font prompts. You can keep a local `experiment_data/S24/README_S24_SETUP.md` with lab-specific commands; it is gitignored like the rest of the cohort directory.
