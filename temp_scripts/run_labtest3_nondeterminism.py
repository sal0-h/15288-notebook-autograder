"""Run assignment grading multiple times and measure nondeterminism.

Programmatic only (no argparse):
- Loads LabTest_3_S26 config from output/LabTest_3_S26/config.yaml
- Creates an in-code config object copied from that runtime config
- Runs the batch orchestrator (parallel workers from config) 5 times
- Stores each run in an isolated output folder (no overwrite of base results)
- Computes score/feedback stability metrics across runs
"""
...
# Edit this block for quick experiments without touching output/*/config.yaml.
EXPERIMENT = {
    "assignment_name": "LabTest_3_S26",
    "source_config_path": None,  # e.g. PROJECT_ROOT / "output" / "HW1" / "config.yaml"
    "experiment_name": "gpt-5",  # Included in output folder name.
    "runs": 5,
...
def _source_config_path() -> Path:
    configured = EXPERIMENT.get("source_config_path")
    if configured:
        return Path(configured)
    assignment = str(EXPERIMENT["assignment_name"])
    return PROJECT_ROOT / "output" / assignment / "config.yaml"


SOURCE_CONFIG_PATH = _source_config_path()
...
def _load_base_config() -> dict:
    ...
    overrides = EXPERIMENT.get("config_overrides") or {}
    if overrides:
...
def _build_experiment_name(base_cfg: dict) -> str:
    ts = datetime.now().strftime("%Y%m%d")
    assignment = _slug(base_cfg.get("assignment_name", "assignment"))
    model = _slug(base_cfg.get("model", DEFAULT_MODEL))
    workers = int(base_cfg.get("workers", 1))
    runs = int(EXPERIMENT.get("runs", 5))
    label = _slug(EXPERIMENT.get("experiment_name", "nondeterminism"))
    return f"{label}__{assignment}__{model}__w{workers}__r{runs}__{ts}"
...
def _run_once(run_index: int, run_config: dict, run_output_dir: Path) -> RunArtifact:
    ...
    assignment_name = str(run_config.get("assignment_name", "LabTest_3_S26"))
...
def _load_existing_run_artifact(run_index: int, run_output_dir: Path) -> RunArtifact | None:
    ...
    # In strict mode, only completed runs are reusable.
...
def _write_outputs(
    ...
    base_results_path = Path(source_config_path).parent / "graded_results.json"
    md_lines = [
        f"# Nondeterminism Report ({assignment_name or 'Assignment'})",
        "",
        f"- Runs: **{len(runs)}**",
        f"- Model: **{model}**",
    ...
    print(f"Experiment artifacts written to: {exp_root}")
    print(f"Metrics JSON: {metrics_path}")
    print(f"Report: {exp_root / 'REPORT.md'}")


def run_nondeterminism_experiment() -> None:
    base_cfg = _load_base_config()
    base_output_dir = Path(base_cfg["output_dir"])
    model = str(base_cfg.get("model") or DEFAULT_MODEL)
    workers = int(base_cfg.get("workers", 1))
    ...
    experiment_name = _build_experiment_name(base_cfg)

    exp_root = _prepare_experiment_root(base_output_dir, experiment_name)

    print(
        f"Running nondeterminism experiment for {base_cfg.get('assignment_name')} "
        f"with model={model}, workers={workers}, runs={n_runs}"
    )
    ...
    print(
        "Resume mode: "
        f"resume_existing_runs={resume_existing_runs}, "
        f"salvage_partial_runs={salvage_partial_runs}, "
        f"rerun_incomplete_runs={rerun_incomplete_runs}, "
        f"strict_resume_validation={strict_resume_validation}"
    )
    ...
    for i in range(1, n_runs + 1):
        ...
        if state == "complete" and existing is not None:
            print(
                f"Reusing existing run {i}/{n_runs} -> {run_dir} "
        ...
        if state == "partial" and existing is not None:
            ...
            if salvage_partial_runs:
                print(
                    f"Resuming partial run {i}/{n_runs} ({existing_count}/{expected_students}) -> {run_dir}"
                )
        ...
        print(f"Starting run {i}/{n_runs} -> {run_dir}")
        ...
        if len(artifact.graded_results) != expected_students:
            print(
                ...
                "This run will be treated as partial and resumed next time."
            )
        if completed < n_runs:
            print(
                f"Progress: {completed}/{n_runs} complete | "
    ...
    _write_outputs(
        ...
        model,
        SOURCE_CONFIG_PATH,
        str(base_cfg.get("assignment_name", EXPERIMENT.get("assignment_name", ""))),
        expected_students,
    )


if __name__ == "__main__":
    run_nondeterminism_experiment()
