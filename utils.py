...
def load_config(config_path: Path | None = None) -> dict:
    ...
    if config_path is not None and _is_assignment_config(root_path) and root_path.exists():
        ...
        if assignment_config_path.exists():
            ...
            # Prompts live in root; merge them in
...
def save_config(config: dict, config_path: Path | None = None) -> None:
    ...
    # Save full config to output/{assignment_name}/config.yaml (excluding prompts)
    assignment_cfg = {k: v for k, v in cfg.items() if k != "prompts"}
